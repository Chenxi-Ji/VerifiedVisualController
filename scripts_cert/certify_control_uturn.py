...
sys.path.append(str(ROOT / "scripts_render"))
...
class Config:
    ...
    if region_size == "large":
        pose_lb = np.array([-0.4, -3.4, -0.4, 1.57, 0.0, 0.0])
        pose_ub = np.array([0.4, -2.6, -0.0, 1.57, 0.0, 0.0])
    elif region_size == "mid":
        pose_lb = np.array([-0.3, -3.2, -0.3, 1.57, 0.0, 0.0])
        pose_ub = np.array([0.3, -2.8, -0.1, 1.57, 0.0, 0.0])
    elif region_size == "small":
        pose_lb = np.array([-0.2, -3.6, -0.3, 1.57, 0.0, 0.0])
        pose_ub = np.array([0.2, -3.2, -0.1, 1.57, 0.0, 0.0])
...
if __name__ == "__main__":
    ...
    assert cfg.scene_name == "uturn", "This certification script is configured for scene_name='uturn' only."
