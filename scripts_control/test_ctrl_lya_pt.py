...
if __name__ == "__main__":
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "(it predates the 2026-06-29 vertical-readout upgrade: a vertical "
            "spatial readout was added next to the horizontal one, and the lateral "
            "pool went 1x3 -> 1x4 for TFLite-safe export, growing the head input "
            "88 -> 120). Train new weights with train_ctrl_lya_pt.py "
            "and copy the resulting weights/ctrl_lya_<timestamp>.pt over "
            "weights/ctrl_lya.pt (then re-run export_to_tflite.py)."
        ) from e
